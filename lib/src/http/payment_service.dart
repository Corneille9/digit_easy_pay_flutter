...
import 'package:digit_easy_pay_flutter/src/http/payment_http_client.dart';

import '../models/gateways/qosic_config.dart';

class PaymentService {
  final QosicConfig config;
  ...
  Future<MobilePayResponse> makeMobilePayment(
    QosicPaymentGateway paymentMethod,
    MobilePayRequest mobilePayRequest,
  ) async {
    var response = await client.post(
      path: "/mobile/make-payment",
      data: mobilePayRequest.toMap(),
      queryParameters: {
        "paymentMethod": paymentMethod.toSnakeCase,
        if (config.applicationKey != null) "applicationKey": config.applicationKey,
      },
    );
  ...
  Future<CardPayResponse> makeCardPayment(CardPayRequest cardPayRequest) async {
    var response = await client.post(
      path: "/card/request-payment",
      data: cardPayRequest.toMap(),
      queryParameters: {if (config.applicationKey != null) "applicationKey": config.applicationKey},
    );
  ...
  Future<TransactionStatus> checkTransactionStatus(String reference) async {
    ...
    return TransactionStatus.values.firstWhere(
      (element) => element.name.toLowerCase() == response.data["status"].toString().toLowerCase(),
    );
  }

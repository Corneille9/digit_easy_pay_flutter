               ...
               (decoration ??
                   InputDecoration(
                     ...
                     suffixIcon: Container(
                       ...
                       child: TextButton(onPressed: () {}, child: Icon(Icons.arrow_drop_down)),
                     ),
                   ));
...
class _SelectFormFieldState extends FormFieldState<String> {
  ...
  void initValues() {
    if (_effectiveController?.text != null && _effectiveController?.text != '') {
      ...
      if (_item!.length > 0) {
        _labelController.text = _item!['label']?.toString() ?? _item!['value']!.toString();
  ...
  Future<void> _showSelectFormFieldMenu() async {
    ...
    if (lvPicked != null && lvPicked != value) {
      var i = (widget.items ?? []).where((lmItem) => lmItem['value'].toString() == lvPicked);
  ...
  Future<void> _showSelectFormFieldDialog() async {
    ...
    if (lvPicked is Map<String, dynamic>) {
      _labelController.text = lvPicked['label']?.toString() ?? lvPicked['value']!.toString();
      _effectiveController?.text = lvPicked['value'].toString();

      if (widget.changeIcon && lvPicked['icon'] != null && lvPicked['icon'] != '') {
        setState(() {
          _icon = lvPicked['icon'];
        });
      }
      ...
      PopupMenuItem<String> loItem = PopupMenuItem<String>(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                lmElement['label']?.toString() ?? lmElement['value']!.toString(),
  ...
  RelativeRect _buttonMenuPosition(BuildContext poContext) {
    ...
    final RenderBox loOverlay = Overlay.of(poContext)?.context.findRenderObject() as RenderBox;
    const Offset loOffset = Offset.zero;

    final RelativeRect loPosition = RelativeRect.fromRect(
      Rect.fromPoints(
        loBar.localToGlobal(loBar.size.centerRight(loOffset), ancestor: loOverlay),
        loBar.localToGlobal(loBar.size.centerRight(loOffset), ancestor: loOverlay),
      ),
...
class _ItemPickerDialogState extends State<ItemPickerDialog> {
  ...
  Widget _showEmpty() {
    return const Stack(
      children: <Widget>[
        Align(alignment: Alignment(0, -0.5), child: Icon(Icons.category, size: 50)),
      ],
    );
  }
    ...
    if (lsQuery.length > 2) {
      ...
      setState(() {
        ...
        _lItemListOriginal.forEach((loCredential) {
          lsValue =
              loCredential['label']?.toString().toLowerCase() ??
...
class ListItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return ListView(children: _itemList(context));
  }
